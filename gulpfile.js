...
gulp.task('restore-database', function() {
  return gulp.src('./data/JSON_sample_restore.json')
    .pipe(rename('JSON_sample.json'))
    .pipe(gulp.dest('./data'));
});

gulp.task('serve:api', ['restore-database'], function(done) {
  cp.exec('node ./node_modules/json-server/bin/index.js --watch ./data/JSON_sample.json --port 3001', {stdio: 'inherit'})
    .on('close', done);
});
